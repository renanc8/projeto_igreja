...
#Pessoa que está sem dizimar a mais de 3 meses (ATENÇÃO Revisar!!)
select Nome, Data_dizimo as Membros_sem_dizimar_mais_de_3_meses
from pessoa
left outer join dizimo 
on pessoa.id_pessoa = dizimo.pessoa 
where dizimo.Data_dizimo not between "2022-02-15" and "2022-05-30" or dizimo.Data_dizimo is null
order by Nome asc;

#Pessoas que possuem mais de um cargo
select Nome, count(cargo) as Quantidade_de_Cargos
from pessoa P
inner join pessoa_cargo PC
on P.id_pessoa = PC.Pessoa
